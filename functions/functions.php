...
// if(pg_connect_errno()){
//   echo "The Connection was not established: " . pg_connect_error();
// }
...
function cart(){
  ...
  if(isset($_GET['add_cart'])){
	  ...
	  $run_check_pro = pg_query($con,"select * from cart where product_id='$product_id'");
	
	  if(pg_num_rows($run_check_pro) > 0){
	     echo "";
	  }else{
	    
		$fetch_pro = pg_query($con, "select * from products where product_id='$product_id'");
		
		$fetch_pro = pg_fetch_array($fetch_pro);
		
		$pro_title = $fetch_pro['product_title'];
		
        $run_insert_pro = pg_query($con, "insert into cart (product_id,product_title,ip_address) values ('$product_id','$pro_title','$ip') ");		
...
function total_items(){
	...
	$run_items = pg_query($con,"select * from cart where ip_address='$ip' ");
	
	echo $count_items = pg_num_rows($run_items);
}
function get_ip(){
...
function get_pro_by_cat_id(){
	...
	if(isset($_GET['cat'])){
		...
		$run_cat_pro = pg_query($con, $get_cat_pro);
		
		$count_cats = pg_num_rows($run_cat_pro);
		
		  if($count_cats == 0){
		    echo "<h2 style='padding:20px;'>No products where found in this category!!</h2>";
		  }
		  
		  while($row_cat_pro = pg_fetch_array($run_cat_pro)){
...
function get_pro_by_brand_id(){
	...
	if(isset($_GET['brand'])){
		...
		$run_brand_pro = pg_query($con, $get_brand_pro);
		
		$count_brands = pg_num_rows($run_brand_pro);
		
		  if($count_brands == 0){
		    echo "<h2 style='padding:20px;'>No products where found in this category!!</h2>";
		  }
		  
		  while($row_brand_pro = pg_fetch_array($run_brand_pro)){
